 ...
 * - нативная — про марафон или про наш Телеграм. Она СВЕТИТСЯ, чтобы её
 *   заметили: это прямая просьба владельца.
 *
 * Нативной становится каждая `GAME_NATIVE_EVERY`-я карточка, и они
 * чередуются между собой — то марафон, то Телеграм. Значит каждая тема
 * всплывает примерно раз в восемь карточек, и это НАМЕРЕННО редко:
...
export function getGameCard(cursor: number): GameCard {
  ...
  if (isNativeSlot) {
    // Номер нативной карточки по счёту: 1, 2, 3… Нечётные отдаём Телеграму,
    ...
    if (nativeSlot % 2 === 1) {
      ...
      if (line) return { kind: "chat", label: "Наш Телеграм", text: line };
    } else {
