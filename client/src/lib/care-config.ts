 ...
 *
 * НАСТРОЕНИЕ — ИСКЛЮЧЕНИЕ, И ОНО НЕ ЛОМАЕТ ПРАВИЛО ВЫШЕ (решение владельца
 * 25.07). Настроение поднимается не припасом, а тем, что ребёнок ПОИГРАЛ
 * с призраком в мини-игру (сейчас это 2048, дальше будут другие — все они
 * живут за этой кнопкой). Правило цело, потому что игра не даёт ни голды,
 * ни опыта: она возвращает только настроение. Фармить тут нечего.
 *
 * Зачем так сделано: раньше игра пряталась на вкладке заданий и открывалась
 * только тем, кто закрыл весь день. Владелец проверил на чистом прогрессе
 * и сказал прямо: «ребёнок до этой игры в жизни не доберётся, слишком большой
 * пласт работы». Теперь вход в неё виден с главного экрана всегда.
 *
...
export type CareNeed = {
  ...
  phrases: string[];
  /**
   * Потребность закрывается не припасом, а МИНИ-ИГРОЙ.
   * Такой нужен ровно один — «Настроение». За кнопкой открывается список игр,
   * припасы для неё не покупаются и в магазине не показываются.
   */
  byGame?: boolean;
};
...
export const CARE_SUPPLIES: CareSupply[] = [
  ...
  // Припасов для настроения тут НЕТ и быть не должно: оно поднимается
  // мини-игрой, а не покупкой (см. byGame и разбор в шапке файла).
  // Были «Кубик» и «Приставка» — убраны 25.07 вместе с этой переделкой.
];
