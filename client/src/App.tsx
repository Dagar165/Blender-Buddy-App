...
function AppContent() {
  ...
  /**
   * ⚠️ ЖЕСТ «СВЕРНУТЬ» ЗАБИРАЕТСЯ У ТЕЛЕГРАМА НА ВСЁ ПРИЛОЖЕНИЕ.
   *
   * Как было. Телеграм считает движение пальцем вниз внутри мини-аппа своим
   * жестом сворачивания и перехватывает его РАНЬШЕ страницы. На главном
   * экране от этого нельзя было покрутить призрака: тянешь его вбок или
   * вниз — сворачивается приложение. Функция есть, а воспользоваться ей
   * нельзя. Владелец 27.07: «давай эта система, которая реализована у нас
   * в игре 2048, распространяется на приложение в целом».
   *
   * В игре этот приём уже стоял и работал — здесь ровно он же, только
   * включается один раз при запуске и больше не выключается.
   *
   * ⚠️ ПРИЛОЖЕНИЕ ПОСЛЕ ЭТОГО ВСЁ РАВНО ЗАКРЫВАЕТСЯ — за «загривок»:
   * шапку самого Телеграма над окном мини-аппа тянуть по-прежнему можно,
   * её `disableVerticalSwipes` не трогает. Отнимается только свайп ПО
   * СОДЕРЖИМОМУ, то есть по нашим экранам. Владелец на это и рассчитывал.
   *
   * Метод из Bot API 7.7. На клиенте постарше его просто нет — тогда всё
   * остаётся как было, и в игре по этому же признаку появляются
   * запасные кнопки-стрелки (см. `canOwnSwipes` в `mini-game.tsx`).
   *
   * ⚠️ ПРОВЕРЯТЬ ТОЛЬКО НА ЖИВОМ САЙТЕ И С ПЕРЕЗАПУСКОМ мини-аппа:
   * в браузере жеста нет вовсе, а Телеграм держит старый клиент в кэше
   * до десяти минут. На этом уже обжигались — приняли кэш за «не помогло»
   * и переписали работающий код.
   */
  useEffect(() => {
    try {
      // @ts-ignore
