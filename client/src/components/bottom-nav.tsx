/**
 * НИЖНИЕ ВКЛАДКИ — четыре, и это решение, а не случайность.
 *
 * Пятую заводить НЕ надо: четыре — предел, при котором подписи ещё читаются
 * на узком телефоне, и владелец это уже решал (когда думали про отдельную
 * вкладку базы знаний — отказались). Новый раздел ищи, куда положить внутрь
 * существующих экранов, а не рядом.
 *
 * Панель `fixed`, то есть лежит ПОВЕРХ страниц. Место под неё отведено
 * один раз в `App.tsx` (`pb-[80px]`) — внутри экранов отступ снизу
 * добавлять не надо, он посчитается дважды.
 */
