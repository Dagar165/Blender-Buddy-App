/**
 * ГЛАВНЫЙ ЭКРАН — вкладка «Питомец». Три экрана, переключаются свайпом.
 *
 * ══ ТРИ ЭКРАНА ══
 *
 * 1. Призрак: комната, уход, оранжевая кнопка задания, гардероб.
 * 2. «Мой рост» — `components/growth-section.tsx`.
 * 3. «Движ» — `components/friends-section.tsx`.
 *
 * Справа точки-указатели (`components/screen-dots.tsx`). Разметка трёх
 * экранов и правила прилипания — ниже по файлу, у самой прокрутки.
 *
 * ══ ГДЕ ЛЕЖАТ ТЕКСТЫ (в этом файле их нет) ══
 *
 * - реплики призрака и стадии роста — `lib/pet-config.ts`;
 * - советы по Blender — `lib/tips-config.ts`;
 * - фразы про чат школы — `lib/community-config.ts`;
 * - уход, припасы, скорость убывания — `lib/care-config.ts`;
 * - вещи и слоты одежды — `lib/shop-config.ts`.
 *
 * ══ ТРИ ПРАВИЛА, КУПЛЕННЫЕ ДОРОГО. НЕ НАРУШАТЬ ══
 *
 * 1. **Призрак не уменьшается.** Ни от длины реплики, ни от высоты
 *    телефона. Нужно место — убирай другие блоки, а не его.
 * 2. **У призрака ДВА верха:** верх картинки и верх одежды. Холст одежды
 *    на 18% выше (`CLOTHING_FRAME`), то есть шляпа торчит примерно на 43
 *    точки над картинкой. Проверяя, не наезжает ли что-то сверху, мерить
 *    надо по одежде.
 * 3. **Экран обязан помещаться в окно.** Никаких блоков, раскрывающихся
 *    в высоту: они ломают переключение свайпом. Длинный список — только
 *    отдельным полноэкранным окном через `createPortal`.
 *
 * Полный разбор всех трёх — `JKids_Bot_как_работать_25.07.md`, разделы 5б и 5в.
 */
      ...
      <div ref={screensRef} className="h-full overflow-y-auto snap-y snap-mandatory">
        ...
        <section className="relative snap-start min-h-full px-5 pb-9 flex flex-col items-center justify-center gap-2">
          ...
          >
             ...
             * ~58 — полоса, в которой живёт облако.
             *
             * ⚠️ ПОЛОСА ШИРЕ, КОГДА ПРИЗРАК ЗОВЁТ В ЧАТ. Под облаком тогда
             * появляется ещё и кнопка «Открыть чат школы», и в обычную
             * полосу она не влезала — садилась призраку прямо на лицо
             * (владелец прислал скриншот). Поэтому отступ считается
             * по тому, есть кнопка или нет.
             *
             * Такая фраза выпадает редко (раз в 18 заходов), и в эти разы
