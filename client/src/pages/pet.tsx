      ...
      <div ref={screensRef} className="h-full overflow-y-auto snap-y snap-mandatory">
        ...
        <section className="snap-start min-h-full px-5 pb-4 flex flex-col items-center justify-center gap-3">
          ...
          {/* Подсказка, что экран не кончился.
              ...
              ⚠️ `sticky bottom-0` — не украшение, а починка. Сначала строчка
              просто стояла последней в колонке, и тестировщик-подросток
              её не увидел вообще: «и на компьютере, и на телефоне не было
              видно, если чуть-чуть скроллил — видно маленький текстик».
              Так и есть: призрак жёсткого размера, экран выше окна, и всё
              последнее оказывается ЗА нижним краем — то есть подсказка
              «тут есть продолжение» сама пряталась за тем самым краем,
              про который хотела рассказать.

              Прилипнув к низу, она видна всё время, пока открыт первый
              экран, и уезжает вместе с ним. */}
          <div className="sticky bottom-0 z-10 flex flex-col items-center pt-1 pb-1 text-slate-500 dark:text-slate-400">
