/**
 * ЭКРАН ЗАДАНИЙ — самый большой файл среди экранов. Стоит сориентироваться
 * до того, как что-то править.
 *
 * ══ ЧТО НА ЭКРАНЕ ══
 *
 * Две вкладки внутри страницы:
 * - «День» — короткие задания на сегодня плюс квиз-разминка;
 * - «Неделя» — проект недели: одна вещь, разбитая на пять шагов по дням.
 *
 * ══ ИЗ ЧЕГО СОБРАН ФАЙЛ (сверху вниз) ══
 *
 * - `QuestCard`  — одна карточка задания: текст, награда, кнопка «Выполнил»;
 * - `WeekPath`   — дорожка из пяти шагов проекта со статусами;
 * - `QuestsPage` — сама страница: вкладки, отправка на проверку, квиз.
 *
 * ══ ГДЕ ЧТО МЕНЯТЬ — СЮДА ЛЕЗТЬ НЕ НАДО ══
 *
 * Текстов заданий здесь НЕТ, они лежат отдельно:
 * - ежедневные и на выходные — `lib/quests-config.ts`;
 * - проекты недели и их шаги — `lib/projects-config.ts`;
 * - вопросы квиза — `lib/quiz-config.ts`;
 * - кто какое задание получает сегодня — `lib/quests-rotation.ts`.
 *
 * Здесь только показ и отправка. Меняешь текст — меняй конфиг, не экран.
 *
 * ══ ГЛАВНОЕ ПРАВИЛО МЕХАНИКИ, КОТОРОЕ ЛЕГКО СЛОМАТЬ ══
 *
 * **Награда начисляется НЕ по нажатию «Выполнил».** Нажатие лишь отправляет
 * заявку живому куратору (`lib/quest-claim.ts` → серверок в папке `worker/`).
 ...
 * «закрытые вопросы» файла `JKids_Bot_открытые_вопросы.md`.
 */
...
export default function QuestsPage() {
  ...
  const nextStepLocked = useMemo(() => {
    ...
    // В выходные календарь открыт до конца, поэтому «завтра» тут не бывает:
    // getPaceIndex вернёт последний шаг и условие ниже не сработает.
