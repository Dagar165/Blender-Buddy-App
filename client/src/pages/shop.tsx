/**
 * МАГАЗИН — единственное место, где тратится голда.
 *
 * ══ ТРИ ВКЛАДКИ ══
 *
 * - «Припасы» — еда и мытьё для призрака, расходуются;
 * - «Одежда» — вещи на призрака, покупаются один раз навсегда;
 * - «Помощь» — заморозка серии и зелье двойной награды.
 *
 * ══ ГДЕ ЧТО МЕНЯТЬ — НЕ ЗДЕСЬ ══
 *
 * Товаров, цен и картинок в этом файле НЕТ:
 * - вещи, цены, слоты одежды — `lib/shop-config.ts`;
 * - припасы и сколько они восстанавливают — `lib/care-config.ts`.
 *
 * Здесь только витрина и сама покупка.
 *
 * ══ ЧТО НАДО ЗНАТЬ ПРО ПОКУПКИ ══
 *
 * **Покупки хранятся ПО НАЗВАНИЮ вещи, а не по её id** (поле `inventory`
 * в сторе). Поэтому переименование вещи в `shop-config.ts` делает старую
 * покупку «неизвестной»: она не пропадёт, но потеряет свой значок
 * и превратится в кубок. Для десяти учеников это терпимо, но знать надо.
 *
 * Голду снимает стор (`hooks/use-game-state.ts`), а не этот экран.
 * Экран только спрашивает и показывает ответ.
 */
